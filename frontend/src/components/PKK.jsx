...
export default function PKK() {
  // Ganti heroImg dengan images.lembaga.pkk
  const heroImg = images.lembaga.pkk;
  ...
  useEffect(() => {
    const fetchPKK = async () => {
      ...
      try {
        ...
        const pkk = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'pkk');
        if (!pkk) throw new Error('Data PKK tidak ditemukan di server');
        setLembaga(pkk);
        setTentangEdit(pkk.tentang || '');
        setVisiEdit(pkk.visi || '');
        setMisiEdit(pkk.misi || '');
        setPengurusEdit(pkk.pengurus || []);
        setUnitEdit(pkk.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data PKK');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data PKK tidak ditemukan</div>;
    ...
    <div className="min-h-screen bg-white pb-10">
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full py-10 md:py-20">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">PKK</h1>
          <p className="max-w-2xl mx-auto text-lg md:text-2xl font-medium drop-shadow mb-4 md:mb-8">Pemberdayaan Kesejahteraan Keluarga</p>
        </div>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang PKK di sini..."
                />
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-l-4 border-green-500">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Tulis visi PKK di sini..."
                />
              </div>
