...
export default function KarangTaruna() {
  // Ganti heroImg dengan images.lembaga.karangtaruna
  const heroImg = images.lembaga.karangtaruna;
  ...
  // Fetch KarangTaruna data from backend
  useEffect(() => {
    const fetchKarangTaruna = async () => {
      setLoading(true);
      setError(null);
      try {
        const data = await publicApiCall('/lembaga');
        const karangTaruna = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'karang taruna');
        if (!karangTaruna) throw new Error('Data Karang Taruna tidak ditemukan di server');
        setLembaga(karangTaruna);
        setTentangEdit(karangTaruna.tentang || '');
        setVisiEdit(karangTaruna.visi || '');
        setMisiEdit(karangTaruna.misi || '');
        setPengurusEdit(karangTaruna.pengurus || []);
        setUnitEdit(karangTaruna.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data Karang Taruna');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data Karang Taruna tidak ditemukan</div>;
    ...
    <div className="min-h-screen bg-white pb-10">
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full py-10 md:py-20">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">KARANG TARUNA</h1>
          <p className="max-w-2xl mx-auto text-lg md:text-2xl font-medium drop-shadow mb-4 md:mb-8">Organisasi Kepemudaan Desa</p>
        </div>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang Karang Taruna di sini..."
                />
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-l-4 border-green-500">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Tulis visi Karang Taruna di sini..."
                />
              </div>
