...
export default function KelompokTani() {
  // Ganti heroImg dengan images.lembaga.kelompoktani
  const heroImg = images.lembaga.kelompoktani;
  ...
  // Fetch KelompokTani data from backend
  useEffect(() => {
    const fetchKelompokTani = async () => {
      ...
      try {
        ...
        const kelompokTani = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'kelompok tani');
        if (!kelompokTani) throw new Error('Data Kelompok Tani tidak ditemukan di server');
        setLembaga(kelompokTani);
        setTentangEdit(kelompokTani.tentang || '');
        setVisiEdit(kelompokTani.visi || '');
        setMisiEdit(kelompokTani.misi || '');
        setPengurusEdit(kelompokTani.pengurus || []);
        setUnitEdit(kelompokTani.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data Kelompok Tani');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data Kelompok Tani tidak ditemukan</div>;
    ...
    <div className="min-h-screen bg-white pb-10">
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full py-10 md:py-20">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">KELOMPOK TANI</h1>
          <p className="max-w-2xl mx-auto text-lg md:text-2xl font-medium drop-shadow mb-4 md:mb-8">Organisasi Petani Desa</p>
        </div>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang Kelompok Tani di sini..."
                />
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-l-4 border-green-500">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Tulis visi Kelompok Tani di sini..."
                />
              </div>
