...
export default function DharmaWanita() {
  ...
  // Fetch DharmaWanita data from backend
  useEffect(() => {
    const fetchDharmaWanita = async () => {
      setLoading(true);
      setError(null);
      try {
        const data = await publicApiCall('/lembaga');
        const dharmaWanita = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'dharma wanita');
        if (!dharmaWanita) throw new Error('Data Dharma Wanita tidak ditemukan di server');
        setLembaga(dharmaWanita);
        setTentangEdit(dharmaWanita.tentang || '');
        setVisiEdit(dharmaWanita.visi || '');
        setMisiEdit(dharmaWanita.misi || '');
        setPengurusEdit(dharmaWanita.pengurus || []);
        setUnitEdit(dharmaWanita.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data Dharma Wanita');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data Dharma Wanita tidak ditemukan</div>;
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang Dharma Wanita di sini..."
                />
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-l-4 border-green-500">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Tulis visi Dharma Wanita di sini..."
                />
              </div>
