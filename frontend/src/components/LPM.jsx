...
export default function LPM() {
  // Ganti heroImg dengan images.lembaga.lpm
  const heroImg = images.lembaga.lpm;
  ...
  // Fetch LPM data from backend
  useEffect(() => {
    const fetchLPM = async () => {
      setLoading(true);
      setError(null);
      try {
        const data = await publicApiCall('/lembaga');
        const lpm = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'lpm');
        if (!lpm) throw new Error('Data LPM tidak ditemukan di server');
        setLembaga(lpm);
        setTentangEdit(lpm.tentang || '');
        setVisiEdit(lpm.visi || '');
        setMisiEdit(lpm.misi || '');
        setPengurusEdit(lpm.pengurus || []);
        setUnitEdit(lpm.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data LPM');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data LPM tidak ditemukan</div>;
    ...
    <div className="min-h-screen bg-white pb-10">
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full py-10 md:py-20">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">LPM</h1>
          <p className="max-w-2xl mx-auto text-lg md:text-2xl font-medium drop-shadow mb-4 md:mb-8">Lembaga Pemberdayaan Masyarakat</p>
        </div>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang LPM di sini..."
                />
              </div>
