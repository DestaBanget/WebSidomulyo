...
export default function LINMAS() {
  // Ganti heroImg dengan images.lembaga.linmas
  const heroImg = images.lembaga.linmas;
  ...
  // Fetch LINMAS data from backend
  useEffect(() => {
    const fetchLINMAS = async () => {
      setLoading(true);
      setError(null);
      try {
        const data = await publicApiCall('/lembaga');
        const linmas = (data.lembaga || []).find(l => l.nama_lembaga && l.nama_lembaga.toLowerCase() === 'linmas');
        if (!linmas) throw new Error('Data LINMAS tidak ditemukan di server');
        setLembaga(linmas);
        setTentangEdit(linmas.tentang || '');
        setVisiEdit(linmas.visi || '');
        setMisiEdit(linmas.misi || '');
        setPengurusEdit(linmas.pengurus || []);
        setUnitEdit(linmas.unit_kegiatan || []);
      } catch (err) {
        setError(err.message || 'Gagal mengambil data LINMAS');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!lembaga) return <div className="text-center py-20 text-red-600">Data LINMAS tidak ditemukan</div>;
    ...
    <div className="min-h-screen bg-white pb-10">
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full py-10 md:py-20">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 drop-shadow-lg">LINMAS</h1>
          <p className="max-w-2xl mx-auto text-lg md:text-2xl font-medium drop-shadow mb-4 md:mb-8">Perlindungan Masyarakat</p>
        </div>
      </div>
      {/* Content Section */}
          ...
          {editMode ? (
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Tulis tentang LINMAS di sini..."
                />
              </div>
              {/* Visi Section */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-l-4 border-green-500">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Tulis visi LINMAS di sini..."
                />
              </div>
              {/* Misi Section */}
            ...
            <>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-l-4 border-primary mb-8">
                <div className="font-bold text-primary text-lg mb-3">TENTANG LINMAS</div>
