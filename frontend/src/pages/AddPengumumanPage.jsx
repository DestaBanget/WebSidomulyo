...
import { FaCalendarAlt, FaHeading, FaAlignLeft, FaBullhorn } from 'react-icons/fa';
import { useAuth } from '../contexts/AuthContext';
import { usePengumuman } from '../contexts/PengumumanContext';
import logokamera from '/logokamera.svg';

const kategoriList = [
  'Umum', 'Penting', 'Darurat', 'Informasi', 'Layanan', 'Kesehatan', 'Pendidikan'
];
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <div className="space-y-3">
            <button
              onClick={() => navigate('/publikasi/pengumuman')}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => navigate('/publikasi/pengumuman'), 1200);
      } else {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="relative w-full h-[400px] md:h-[500px] flex items-center justify-center overflow-hidden" style={{
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full animate-fade-in">
          <h1 className="text-5xl md:text-6xl font-extrabold text-white mb-4 drop-shadow-lg">Tambah Pengumuman</h1>
          <p className="text-white text-lg md:text-xl font-medium mb-6 drop-shadow text-center max-w-2xl">Isi form berikut untuk menambah pengumuman desa</p>
        </div>
      </div>
      {/* Form Card */}
      <div className="flex-1 flex flex-col items-center justify-center py-12 px-2 md:px-4">
        <div className="mb-8 text-center">
          <h2 className="text-2xl md:text-3xl font-extrabold text-primary mb-2 tracking-tight drop-shadow">Form Tambah Pengumuman</h2>
          <p className="text-gray-500 text-base md:text-lg max-w-xl mx-auto">Silakan lengkapi data berikut untuk menambah pengumuman desa. Pastikan semua data yang diinput sudah benar sebelum disimpan.</p>
        </div>
        <div className="max-w-2xl w-full bg-white/95 rounded-3xl shadow-2xl border border-blue-100 p-8 md:p-12 mx-auto mt-8 md:mt-12 animate-slide-up transition-all duration-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Judul Pengumuman <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Gambar Pengumuman</label>
              <div className="flex flex-col items-center gap-3">
                ...
                {!imgPreview && <span className="text-gray-400 text-sm">Pilih gambar pengumuman (jpg/png)</span>}
              </div>
            </div>
            <div>
            ...
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Isi Pengumuman <span className="text-red-500">*</span></label>
              <div className="relative">
                <span className="absolute left-3 top-3 text-primary/70"><FaBullhorn size={16} /></span>
                <textarea 
                  className="w-full border-2 border-primary/20 rounded-lg px-10 py-2 focus:outline-none focus:border-primary focus:shadow-lg transition placeholder:text-gray-400 text-lg" 
                  rows={5} 
            ...
            <button 
              ...
              disabled={loading}
            >
              <FaBullhorn size={20} /> {loading ? 'Menyimpan...' : 'Simpan Pengumuman'}
            </button>
          </form>
        </div>
      </div>
